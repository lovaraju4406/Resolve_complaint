...
export default function AgentInfo({ showToast }) {
  ...
  const openScorecard = async agent => {
    ...
    try {
      ...
      const csat     = perf.avgCsat || (Math.random()*2+3).toFixed(1);
      ...
      setScorecard({agent, data:{total:0,resolved:0,pending:0,progress:0,resRate:0,csat:0,avgResponse:'—'}});
    }
      ...
      <div className="ag-wrap">
        <div className="ag-stats">
          {[
            ...
            {label:'Results',      num:filtered.length,                        color:'#6366f1'},
          ].map(s=>(
                  ...
                  {paginated.map(agent=>{
                    ...
                    return(
                      <tr key={agent._id} className={isEditing?'ag-edit-row':''}>
                        ...
                        <td>{isEditing?<input className="ag-edit-input" type="email" value={editForm.email} onChange={e=>setEditForm(p=>({...p,email:e.target.value}))}/>:agent.email}</td>
                        <td>{isEditing?<input className="ag-edit-input" type="tel" value={editForm.phone} placeholder="Phone number" onChange={e=>setEditForm(p=>({...p,phone:e.target.value}))}/>:(agent.phone||'—')}</td>
                        <td>
                          <div className="ag-status-toggle" onClick={()=>toggleActive(agent)}>
                            <div className={`ag-toggle-track ${agent.isActive?'on':'off'}`}><div className="ag-toggle-thumb"/></div>
                            <span className="ag-status-label" style={{color:agent.isActive?'#059669':'#9ca3af'}}>{agent.isActive?'Active':'Inactive'}</span>
                          </div>
                        </td>
                        <td>
                          <div className="ag-action-row">
                            {isEditing?(
                              <><button className="ag-btn save" onClick={()=>saveEdit(agent._id)}>✓ Save</button><button className="ag-btn cancel" onClick={()=>setEditId(null)}>✕</button></>
                            ):(
                              <><button className="ag-btn edit" onClick={()=>startEdit(agent)}>✏ Edit</button><button className="ag-btn scorecard" onClick={()=>openScorecard(agent)}>📊 Score</button><button className="ag-btn del" onClick={()=>deleteAgent(agent._id)}>🗑</button></>
                            )}
                          </div>
                        </td>
                      </tr>
          ...
          {totalPages>1&&(
            <div className="ag-pagination">
              ...
              <div className="ag-page-btns">
                ...
                {Array.from({length:totalPages},(_,i)=><button key={i+1} className={`ag-page-btn${page===i+1?' active':''}`} onClick={()=>setPage(i+1)}>{i+1}</button>)}
          ...
          <div className="ag-modal">
            ...
            {[
              ...
              <div className="ag-modal-field" key={f.key}>
                ...
                <input className="ag-modal-input" type={f.type} placeholder={f.ph} value={newAgent[f.key]} onChange={e=>setNewAgent(p=>({...p,[f.key]:e.target.value}))}/>
              </div>
            ))}
            ...
            <div className="ag-score-hero">
              ...
              <div>
                ...
                <div style={{display:'flex',alignItems:'center',gap:6,marginTop:6}}>
                  <div className={`ag-toggle-track ${scorecard.agent.isActive?'on':'off'}`} style={{transform:'scale(.8)',transformOrigin:'left'}}><div className="ag-toggle-thumb"/></div>
                  <span style={{fontSize:12,fontWeight:700,color:scorecard.agent.isActive?'#059669':'#9ca3af'}}>{scorecard.agent.isActive?'Active':'Inactive'}</span>
                </div>
              </div>
            </div>
            {scorecardLoading?(
              ...
              <>
                ...
                {[
                  ...
                  <div key={m.label} style={{marginBottom:14}}>
                    <div style={{display:'flex',justifyContent:'space-between',fontSize:13,fontWeight:700,color:'#374151',marginBottom:6}}><span>{m.label}</span><span style={{color:m.color}}>{m.value}</span></div>
                    <div className="ag-perf-bar"><div className="ag-perf-fill" style={{width:`${m.pct}%`,background:`linear-gradient(90deg,${m.color},${m.color}88)`}}/></div>
                  </div>
                ))}
