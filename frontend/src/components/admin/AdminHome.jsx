...
/* ── Forecast chart bars ── */
.forecast-bar{display:flex;align-items:flex-end;gap:8px;height:100px;padding:0 4px}
.forecast-bar-wrap{flex:1;display:flex;flex-direction:column;align-items:center;gap:4px}
.forecast-bar-fill{width:100%;border-radius:4px 4px 0 0;transition:height .5s ease}
.forecast-bar-lbl{font-size:9px;color:var(--muted)}
...
/* ────────────────────────────────────────────────────────
   AUDIT LOG — records every admin action in localStorage
──────────────────────────────────────────────────────── */
...
function Dashboard({ onNav, unreadCount }) {
  ...
  const rawC = c.data;
  const rawA = a.data;
  const complaints = Array.isArray(rawC) ? rawC : (rawC?.complaints || rawC?.data || []);
  const agentsArr  = Array.isArray(rawA) ? rawA : (rawA?.agents   || rawA?.data || []);
  setComplaints(complaints);
  setAgents(agentsArr);
      setLoading(false);
      // Anomaly detection: spike in last 24h vs avg
      const now = Date.now();
      const last24h = complaints.filter(x => now - new Date(x.createdAt).getTime() < 86400000).length;
      const dailyAvg = complaints.length / 7;
      if (last24h > dailyAvg * 2.5 && last24h > 3) {
        setAnomaly({ type:'spike', count:last24h, avg:Math.round(dailyAvg) });
      }
      const cities = {};
      ...
      {anomaly && (
        <div style={{ background:'#fff7ed', border:'1.5px solid rgba(217,119,6,.3)', borderRadius:14, padding:'14px 18px', marginBottom:18, display:'flex', alignItems:'flex-start', gap:12 }}>
          ...
          <div>
            ...
            {anomaly.type==='spike' && <div style={{ fontSize:13, color:'#b45309' }}>Complaint spike: <strong>{anomaly.count}</strong> complaints in the last 24h (avg: {anomaly.avg}/day). Consider adding agents.</div>}
            {anomaly.type==='city' && <div style={{ fontSize:13, color:'#b45309' }}>High concentration from <strong>{anomaly.city}</strong> — {anomaly.count} complaints. Investigate location-specific issues.</div>}
          </div>
...
function AnalyticsPage() {
  ...
  const rawC = c.data;
  const rawA = a.data;
  setComplaints(Array.isArray(rawC) ? rawC : (rawC?.complaints || rawC?.data || []));
  setAgents(Array.isArray(rawA) ? rawA : (rawA?.agents || rawA?.data || []));
  setLoading(false);
});
  }, []);
  ...
  for(let i=29;i>=0;i--){
    ...
    monthData.push({l:i%5===0?d.getDate().toString():'',date:d.toDateString(),v:0});
  }
  ...
  const forecast = Array.from({length:7},(_,i)=>({
    l:`D+${i+1}`, v:Math.max(0,Math.round((last7/7)*(1+growthRate*(i+1)/7)))
  }));
  ...
  const generateAiSummary = async () => {
    ...
    } catch { setAiSummary('AI summary unavailable. Check API connection.'); }
    setAiLoading(false);
  ...
  const generateForecast = () => {
    addAuditEntry('Forecast Viewed', 'Admin viewed 7-day complaint volume prediction');
  };
    ...
    <>
      ...
      <div className="ai-banner">
        ...
        <div style={{ flex:1 }}>
          ...
          {aiSummary ? (
            <div className="ai-banner-desc">{aiSummary}</div>
          ) : (
            <div className="ai-banner-desc">Get a 3-sentence AI-generated summary of your current complaint landscape and recommended actions.</div>
          )}
            ...
            topCats.map(([cat,count])=>{
              ...
              return (
                <div key={cat} style={{marginBottom:13}}>
                  <div style={{display:'flex',justifyContent:'space-between',fontSize:13,fontWeight:600,color:'var(--text)',marginBottom:5}}>
                    <span>{cat}</span><span style={{color:'var(--muted)'}}>{count} ({pct}%)</span>
                  </div>
                  <div style={{height:7,background:'var(--bg2)',borderRadius:4,overflow:'hidden'}}>
                    <div style={{height:'100%',borderRadius:4,background:'linear-gradient(90deg,#2D6A4F,#40916C)',width:`${pct}%`,transition:'width .6s ease'}}/>
                  </div>
                </div>
              );
            })
      ...
      <div className="a-card">
        <div className="a-card-hdr">
          <span className="a-card-title">🔮 7-Day Complaint Volume Forecast</span>
          <button className="a-card-link" onClick={generateForecast}>Refresh</button>
        </div>
        <div style={{fontSize:12.5,color:'var(--muted)',marginBottom:14}}>Based on recent 14-day trend growth rate of {growthRate>=0?'+':''}{(growthRate*100).toFixed(1)}%/week.</div>
        <div style={{display:'flex',alignItems:'flex-end',gap:8,height:100,padding:'0 4px'}}>
          {forecast.map((f,i)=>{
            const maxV=Math.max(...forecast.map(x=>x.v),1);
            const h=Math.max(f.v>0?(f.v/maxV)*80:4,4);
            return (
...
function WorkloadView() {
  const [agents,setAgents]=useState([]);const[wl,setWl]=useState({});const[loading,setLoading]=useState(true);
  useEffect(()=>{const load=async()=>{try{const r=await axios.get(`${API}/AgentUsers`);const all=r.data||[];const w={};await Promise.all(all.map(async ag=>{try{const res=await axios.get(`${API}/allcomplaints/${ag._id}`);const d=res.data||[];w[ag._id]={total:d.length,pending:d.filter(c=>c.status==='Pending').length,progress:d.filter(c=>c.status==='In Progress').length,resolved:d.filter(c=>c.status==='Resolved'||c.status==='completed').length};}catch{w[ag._id]={total:0,pending:0,progress:0,resolved:0};}}));setAgents(all);setWl(w);}catch{}setLoading(false);};load();},[]);
  ...
  return(<>
    ...
    {loading?<div className="a-wl-grid">{[1,2,3,4].map(i=><div key={i} className="a-skel" style={{height:148}}/>)}</div>:agents.length===0?<div className="a-empty"><div className="a-empty-ico">👥</div><div className="a-empty-title">No agents registered yet</div></div>:(
      <div className="a-wl-grid">{agents.map(ag=>{const w=wl[ag._id]||{total:0,pending:0,progress:0,resolved:0};const pct=maxL>0?Math.round((w.total/maxL)*100):0;const col=pct>75?'#B91C1C':pct>40?'#92400E':'#2D6A4F';const bg=pct>75?'#FEF2F2':pct>40?'#FEF3C7':'#D8F3DC';return(<div key={ag._id} className="a-wl-card" style={{borderColor:`${col}28`}}><div className="a-wl-top"><div className="a-wl-agent"><div className="a-wl-av">{ag.name?.[0]?.toUpperCase()}</div><div><div className="a-wl-name">{ag.name}</div><div className="a-wl-email">{ag.email}</div></div></div><span className="a-wl-badge" style={{background:bg,color:col,border:`1px solid ${col}28`}}>{w.total} tickets</span></div><div className="a-wl-track"><div className="a-wl-fill" style={{width:`${pct}%`,background:`linear-gradient(90deg,${col},${col}aa)`}}/></div><div className="a-wl-meta"><span>⏳ {w.pending} pending</span><span>🔄 {w.progress} active</span><span>✅ {w.resolved} done</span></div></div>);})}</div>
    )}
  </>);
}
...
function CategoryManager() {
  const [cats, setCats] = useState(() => {
    try { return JSON.parse(localStorage.getItem('resolvenow_cats') || 'null') || DEFAULT_CATS; } catch { return DEFAULT_CATS; }
  });
  ...
  const save = () => {
    localStorage.setItem('resolvenow_cats', JSON.stringify(cats));
    showToast('Categories saved', 'success');
    addAuditEntry('Categories Updated', `Admin saved category changes`);
  };
  ...
  const submit = () => {
    ...
    if (editCat) {
      setCats(p=>p.map(c=>c.id===editCat.id?{...c,...form}:c));
      addAuditEntry('Category Edited', form.name);
    } else {
      setCats(p=>[...p,{id:Date.now(),...form}]);
      addAuditEntry('Category Added', form.name);
    }
    setShowModal(false);
  };
  ...
  return (
    <>
      ...
      <div className="a-pg-sub">Create and manage complaint categories. SLA is the default deadline in hours for each category.</div>
          ...
          <div key={cat.id} className="cat-card">
            <div style={{ display:'flex', alignItems:'center', gap:10 }}>
              ...
              <div>
                ...
                <div className="cat-sla">SLA: {cat.sla}h deadline</div>
              </div>
            </div>
      ...
      {showModal && (
        <div className="a-modal-ov" onClick={e=>e.target===e.currentTarget&&setShowModal(false)}>
          <div className="a-modal">
            ...
            {[
              { label:'Category Name', key:'name', type:'text',   ph:'e.g. Water Supply' },
              { label:'Icon (emoji)',  key:'icon', type:'text',   ph:'e.g. 💧'           },
              { label:'SLA (hours)',   key:'sla',  type:'number', ph:'e.g. 48'           },
            ].map(f=>(
              <div className="a-form-field" key={f.key}>
                <label className="a-form-label">{f.label}</label>
                <input className="a-form-input" type={f.type} placeholder={f.ph}
                  value={form[f.key]} onChange={e=>setForm(p=>({...p,[f.key]:f.type==='number'?parseInt(e.target.value)||0:e.target.value}))}/>
              </div>
            ))}
...
function AuditLogView() {
  ...
  const filtered = log.filter(e =>
    !filter || e.action.toLowerCase().includes(filter.toLowerCase()) || e.detail.toLowerCase().includes(filter.toLowerCase())
  );

  const clearLog = () => {
    if (!window.confirm('Clear entire audit log?')) return;
  ...
  const exportLog = () => {
    const csv = ['Action,Detail,Timestamp', ...log.map(e => `"${e.action}","${e.detail}","${new Date(e.at).toLocaleString('en-IN')}"`)].join('\n');
  ...
  return (
    <>
      ...
      <div className="a-pg-sub">Complete record of every admin action — assignments, status changes, user management, and more.</div>

      <div style={{ display:'flex', gap:10, marginBottom:16, flexWrap:'wrap' }}>
        <input style={{ flex:1, minWidth:200, padding:'9px 13px', borderRadius:10, border:'1.5px solid var(--border)', background:'var(--bg2)', fontFamily:'var(--font)', fontSize:13, color:'var(--text)', outline:'none' }}
          placeholder="Filter by action or detail…" value={filter} onChange={e=>setFilter(e.target.value)}/>
...
export default function AdminHome() {
  ...
  useEffect(() => {
    const fetch = async () => {
      try {
        ...
        setNotifs((r.data||[]).sort((a,b)=>new Date(b.createdAt)-new Date(a.createdAt)).slice(0,8)
  ...
  // Wrap setActive to also log navigation
      ...
      <div className="a-shell">
        <div className="a-content">
          ...
          {active==='complaints' && (<>
            ...
            <div className="a-pg-sub">Review, assign and track all unassigned complaints. Email is sent to user & agent on assignment.</div>
            <div style={{background:'#eff6ff',border:'1.5px solid rgba(37,99,235,.2)',borderRadius:12,padding:'10px 14px',marginBottom:16,fontSize:13,color:'#1e40af',display:'flex',alignItems:'center',gap:8}}>
              ℹ️ <strong>Email notifications</strong> are triggered automatically by the backend when you assign a complaint. Ensure nodemailer is configured in <code>assignmentController.js</code>.
            </div>
          ...
          {active==='agents'    && (<>
            ...
            <div className="a-pg-sub">Add, edit, toggle active/inactive status, and view scorecards for each agent.</div>
